...
from sam2act.mvt.sam2_lora import LoRA_SAM2, LoRA_SAM2_mem
        ...
        if self.ifSAM2:
            ...
            if self.lora_finetune:
                if not self.use_memory:
                    lora_sam2 = LoRA_SAM2(sam2, lora_r)
                else:
                    lora_sam2 = LoRA_SAM2_mem(sam2, lora_r)
                
                self.sam2 = lora_sam2.sam2
                # adapt mvt's dim
                self.sam2.hidden_dim = 128
                # adapt sam2.1
                self.sam2.no_obj_embed_spatial = None
                # self.sam2.train()
                if self.rank == 0:
                    get_model_para(self.sam2.image_encoder)
            else:
                self.sam2 = sam2
                # adapt sam2.1
                self.sam2.no_obj_embed_spatial = None
