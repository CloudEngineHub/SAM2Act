...
import numpy as np
import torch
import sam2act.mvt.aug_utils as aug_utils
...
def apply_se3_aug_con_same(
    ...
    roll = np.deg2rad(rot_aug_range[:, 0:1] * aug_utils.rand_dist_same((bs, 1)))
    pitch = np.deg2rad(rot_aug_range[:, 1:2] * aug_utils.rand_dist_same((bs, 1)))
    yaw = np.deg2rad(rot_aug_range[:, 2:3] * aug_utils.rand_dist_same((bs, 1)))
    rot_shift_3x3 = torch3d_tf.euler_angles_to_matrix(
        torch.cat((roll, pitch, yaw), dim=1), "XYZ"
    )
    rot_shift_4x4 = identity_4x4.detach().clone()
...
def apply_se3_aug_con_sequence(
    ...
    # identity matrix
    identity_4x4 = torch.eye(4).unsqueeze(0).repeat(bs, 1, 1).to(device=device)    # (bs,4,4)
    ...
    if pc_aug:
        pcd = perturb_se3(pcd, trans_shift_4x4, rot_shift_4x4, action_gripper_4x4, bounds)
        return perturbed_action_trans, perturbed_action_quat_xyzw, pcd, trans_shift_4x4, rot_shift_4x4
    else:
        return perturbed_action_trans, perturbed_action_quat_xyzw
    


def apply_se3_aug_given_matrix(
    pcd,
    action_gripper_pose,
    bounds,
    trans_shift_4x4,
    ...
    trans_shift = trans_shift_4x4[:, 0:3, 3]
    # apply bounded translations
    bounds_x_min, bounds_x_max = bounds[:, 0], bounds[:, 3]
    bounds_y_min, bounds_y_max = bounds[:, 1], bounds[:, 4]
    bounds_z_min, bounds_z_max = bounds[:, 2], bounds[:, 5]
